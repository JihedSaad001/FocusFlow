...
export const fetchResources = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/resources/wallpapers`) // Update to fetch wallpapers specifically
...
export const fetchAudioResources = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/resources/ambient-sounds`)
    return response.data
  } catch (error) {
...
export const uploadFile = async (file: File, category: string, tags: string, token: string | null) => {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/resources/upload-wallpaper`, formData, {
      headers: {
...
export const uploadAudio = async (file: File, name: string, tags: string, token: string | null) => {
  if (!token) throw new Error("Unauthorized: No token provided!")
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/resources/upload-audio`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
        Authorization: `Bearer ${token}`,
      },
...
export const fetchUserStats = async (token: string) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.message || `Failed to fetch user statistics: ${response.statusText}`)
    }
...
export const fetchProjectStats = async (token: string) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.message || `Failed to fetch project statistics: ${response.statusText}`)
    }
